...
from constants import (
    ANCHO_PANTALLA, ALTO_PANTALLA, AZUL, BLANCO, FPS,
    ...
    BACKGROUND_IMAGE # ¡Importa la nueva constante para el fondo!
)
...
pygame.display.set_caption("Plataformas con Fondo de Pantalla")

puntuacion = 0
font = pygame.font.Font(None, 36)

# 3. Carga de assets (audio y ahora también el fondo)
import pygame
...
try:
    ...
    # Escala la imagen de fondo para que se ajuste a la pantalla
    background = pygame.transform.scale(background, (ANCHO_PANTALLA, ALTO_PANTALLA))
except pygame.error as message:
    print(f"Error al cargar la imagen de fondo: {BACKGROUND_IMAGE}. Asegúrate de que está en la misma carpeta.")
    print(message)
    background = None # Si no se carga, el fondo será el color azul.


# 4. Creación de objetos del juego
jugador = Jugador()
...
while corriendo:
    ...
    for evento in pygame.event.get():
        ...
        if evento.type == pygame.KEYDOWN:
            ...
            if evento.key == pygame.K_SPACE:
                if jugador.en_suelo and jump_sound:
                    jugador.jump()
                    jump_sound.play()
                else:
                    jugador.jump()
    ...
    # Dibuja el fondo de pantalla primero
    if background:
        pantalla.blit(background, (0, 0))
    else:
        pantalla.fill(AZUL) # Si no hay fondo, usa el color azul.
